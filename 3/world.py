    ...
    with open(file_name) as f:
        ...
        for line in f:
            ...
            for j in range(len(blocks)):
                cell = _Cell(_canvas, blocks[j], j * BLOCK_SIZE,
                             i * BLOCK_SIZE)
                row.append(cell)
            _map.append(row)
            i+=1
...
def destroy(row,col):
    if row < 1 or col < 1 or row >= get_rows()-1 or col >= get_cols():
        return False
    return _map[row][col].destroy()

def inside_of_map(row,col):
    if row < 0 or col < 0 or row >= get_rows()-1 or col >=get_cols():
        return False
    return True
